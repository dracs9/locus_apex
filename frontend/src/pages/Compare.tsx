...
function Row({ label, children }: { label: string; children: ReactNode }) {
  return (
    <tr>
      <th scope="row" className="sticky left-0 z-10 border-t border-border/60 bg-card py-2.5 pr-3 text-left align-top text-xs font-semibold text-muted-foreground">
        {label}
      </th>
      {children}
    </tr>
...
export function Compare() {
  ...
  const cells = (render: (rec: Recommendation, uni: University | undefined) => ReactNode) =>
    columns.map((rec, i) => (
      <td key={rec.university_id} className={cn("border-t border-border/60 px-3 py-2.5 align-top", i === 0 && "bg-primary/5")}>
        ...
        <Card className="p-0">
          <div className="overflow-x-auto rounded-xl">
            ...
            >
              ...
              <thead>
                <tr>
                  <th scope="col" className="sticky left-0 z-20 bg-card px-0 py-3 pr-3 text-left align-bottom text-xs font-semibold text-muted-foreground">
                    {t.compare.param}
                  </th>
                  {columns.map((rec, i) => {
                    const uni = map.get(rec.university_id);
                    return (
                      <th key={rec.university_id} scope="col" className={cn("px-3 py-3 text-left align-top", i === 0 && "bg-primary/5")}>
              ...
              <tfoot>
                <tr>
                  ...
                  {columns.map((rec, i) => (
                    <td key={rec.university_id} className={cn("border-t border-border/60 px-3 py-3 align-top", i === 0 && "bg-primary/5")}>
                      <Button
                        className="w-full"
                        size="sm"
